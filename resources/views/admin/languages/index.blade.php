    ...
    <script type="application/javascript">
        var tableCols = [
            ...
            {"data": "lang"},
            {
                ...
                render: function ( data, type, row, meta ) {
                    var returnhtml = '';
                    @if(Auth::user()->hasPermission("update", "languages"))
                        returnhtml += '<a href="/languages/'+row.id+'" class="btn btn-sm btn-info float-left">{{__("generic.edit")}}</a>';
                    @endif
                    @if(Auth::user()->hasPermission("delete", "languages"))
                        returnhtml += '<a href="/languages/'+row.id+'" class="btn btn-sm btn-danger float-right">{{__("generic.delete")}}</a>';
                    @endif
                    return returnhtml;
                }
            }
        ...
        $(document).ready(function () {
            generateTable($('#tbl_Users'), "/languages", true);
        });
    </script>
@endpush
