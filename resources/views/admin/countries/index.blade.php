        ...
        <div class="table-responsive">
            <table class="table table-condensed dataTable" id="tbl_Users">
                ...
                <tr>
                    ...
                    <th>{{__("generic.country_code")}}</th>
                    <th>{{__("generic.country_iso_code")}}</th>
                    <th>{{__("generic.country_phone_code")}}</th>
                    <th>{{__("generic.country_currency_code")}}</th>
...
@push('scripts')
    ...
    <script type="application/javascript">
        var tableCols = [
            ...
            {"data": "currency_code"},
            {
                ...
                render: function ( data, type, row, meta ) {
                    var returnhtml = '';
                    @if(Auth::user()->hasPermission("update", "countries"))
                        returnhtml += '<a href="/countries/'+row.id+'" class="btn btn-sm btn-info float-left">{{__("generic.edit")}}</a>';
                    @endif
                    @if(Auth::user()->hasPermission("delete", "countries"))
                        returnhtml += '<a href="/countries/'+row.id+'" class="btn btn-sm btn-danger float-right">{{__("generic.delete")}}</a>';
                    @endif
                    return returnhtml;
                }
            }
        ...
        $(document).ready(function () {
            generateTable($('#tbl_Users'), "/countries", true);
        });
    </script>
@endpush
